...
COPY . /go/src/github.com/DjaloS/kaos

WORKDIR /go/src/github.com/DjaloS/kaos/cmd/kaos-operator

RUN CGO_ENABLED=0 GOOS=linux go build -ldflags "-X main.version=0.4 -X main.timestamp=1520033373"

FROM scratch

COPY --from=0 /go/src/github.com/DjaloS/kaos/cmd/kaos-operator/kaos-operator /
